...
Controller::Controller(ros::NodeHandle nh) : m_nh(nh), m_frequency(10)
{
  ...
  m_target_angle = m_nh.subscribe("/controller/input", 1, &Controller::targetCallback, this);
  // Service callback
  ...
  m_control_mode = ControlModes::ANGLE_HOLD;
...
void Controller::targetCallback(const geometry_msgs::Pose &msg)
{
...
void Controller::spin()
{
  ...
  Eigen::Vector6d    tau_anglehold      = Eigen::VectorXd::Zero(6);
  ...
  while (ros::ok())
  {
    ...
    switch (m_control_mode)
    {
      ...
      case ControlModes::ANGLE_HOLD:
      tau_anglehold = angleHold(tau_openloop,
                                position_state,
                                orientation_state,
                                velocity_state,
                                orientation_setpoint);
      tau_command = tau_anglehold;
      break;
    ...
    msg.force.x = linear_target.position.x;
    msg.force.y = linear_target.position.y;
    msg.force.z = linear_target.position.z;
...
Eigen::Vector6d Controller::angleHold(const Eigen::Vector6d &tau_openloop,
                              const Eigen::Vector3d &position_state,
                              const Eigen::Quaterniond &orientation_state,
                              const Eigen::Vector6d &velocity_state,
                              const Eigen::Quaterniond &orientation_setpoint) {
  ...
  bool activate_depthhold = fabs(tau_openloop(PoseIndex::YAW)) < c_normalized_force_deadzone;
  if (activate_depthhold)
  {
    tau = m_controller->getFeedback(Eigen::Vector3d::Zero(), orientation_state, velocity_state,
                                    Eigen::Vector3d::Zero(), orientation_setpoint);

    // Allow only pitch, yaw, roll feedback command
    tau(SURGE)  = 0;
    tau(SWAY) = 0;
    tau(HEAVE)   = 0;
  }
